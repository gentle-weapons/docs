...
```bash
pbctl add "python=3.12"
pbctl add "numpy>=1.24"
```

### Add multiple packages at once

```bash
pbctl add python numpy pandas scikit-learn
```
...
pbctl add --pypi "boto3>=1.28"
```

### Add a platform-specific dependency

```bash
pbctl add cudatoolkit --platform linux-aarch64
```

### Add a dependency to a feature

```bash
pbctl add --feature test pytest pytest-cov
```

### Add a git-based dependency

```bash
pbctl add --git https://github.com/user/repo --branch main mypackage
```

### Add an editable PyPI dependency

```bash
pbctl add --pypi --editable "mylib @ file:///path/to/mylib"
```
