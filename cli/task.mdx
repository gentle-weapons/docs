...
```bash
pbctl task add <NAME> <COMMAND> [OPTIONS]
```
...
```bash
pbctl task remove <NAMES>... [OPTIONS]
```
...
```bash
pbctl task alias <NAME> <DEPENDS_ON>... [OPTIONS]
```
...
```bash
# Add a simple task
pbctl task add build "make build"

# Add a task with a dependency
pbctl task add test "pytest tests/" --depends-on build

# Add a platform-specific task
pbctl task add setup "bash setup.sh" --platform linux-aarch64

# Add a task to a feature
pbctl task add lint "ruff check ." --feature dev

# Add a task with environment variables and a working directory
pbctl task add serve "python -m http.server" --cwd ./dist --env PORT=80

# Add a task with a description
pbctl task add docs "sphinx-build docs/ docs/_build" --description "Build documentation"

# Create an alias that runs multiple tasks
pbctl task alias ci lint test build

# Remove a task
pbctl task remove build

# Remove a platform-specific task
pbctl task remove setup --platform linux-aarch64

# List all tasks
pbctl task list

# List tasks for a specific environment
pbctl task list --environment test

# Show a summary of tasks
pbctl task list --summary
```
